...
function utf8ToAscii_umlauts(_str)
	...
	_str = replaceUtf8(_str, 195164);		-- �
	_str = replaceUtf8(_str, 195132);		-- �
	_str = replaceUtf8(_str, 195182);		-- �
	_str = replaceUtf8(_str, 195150);		-- �
...
function asciiToUtf8_umlauts(_str)
	...
	_str = replaceAscii(_str, 195164);		-- �
	_str = replaceAscii(_str, 195132);		-- �
	_str = replaceAscii(_str, 195182);		-- �
	_str = replaceAscii(_str, 195150);		-- �
...
--		local tmp = database.utf8_ascii[_ascii];
	...
	-- replace special ASCII characters like ���� / hence open.XML() can't handle them
...
function getGameVersion(proc)
	...
	if foundAddress == nil or foundAddress == 0 then
		foundAddress = findPatternInProcess(proc, string.char(0xBD, 0x04, 0xEF, 0xFE), "xxxx", x32Addr, scanSize)
	end
