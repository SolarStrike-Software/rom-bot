...
CSkill = class(function(self, copyfrom)
    self.Address = 0;
    ...
    self.CastTime = 0;
    self.Cooldown = 0;
    self.LastCastTime = { high = 0, low = 0};
...
function CSkill:canUse(_only_friendly, target)
    ...
    local function debug_skilluse(_reason, _arg1, _arg2, _arg3, _arg4, _arg5, _arg6)
        ...
        if (_arg1) then
            hf_arg1 = make_printable(_arg1);
        end
        if (_arg2) then
            hf_arg2 = make_printable(_arg2);
        end
        if (_arg3) then
            hf_arg3 = make_printable(_arg3);
        end
        if (_arg4) then
            hf_arg4 = make_printable(_arg4);
        end
        if (_arg5) then
            hf_arg5 = make_printable(_arg5);
        end
        if (_arg6) then
            hf_arg6 = make_printable(_arg6);
        end

        local msg = sprintf("[DEBUG] %s %s %s %s %s %s %s %s\n", _reason, self.Name, hf_arg1, hf_arg2, hf_arg3, hf_arg4,
            hf_arg5, hf_arg6);
    ...
    player:updatePsi()
    if (player.Mana < self.Mana or player.Rage < self.Rage or player.Energy < self.Energy or player.Focus < self.Focus or
        player.Psi < self.Psi) then
    ...
    if (self.maxuse > 0 and self.used >= self.maxuse) then
    ...
    -- Out of range
    if (player:distanceToTarget() > realRange and self.Target ~= STARGET_SELF) then -- range check only if no selftarget skill
        debug_skilluse("OUTOFRANGE", player:distanceToTarget(), realRange);
        return false;
    end

    -- Too close
    if (player:distanceToTarget() < self.MinRange and self.Target ~= STARGET_SELF) then -- range check only if no selftarget skill
        debug_skilluse("MINRANGE", player:distanceToTarget(), self.MinRange);
        return false;
    end

    -- check pullonly skills
    if (self.pullonly == true and not player.ranged_pull) then
    ...
    if (self.Type == STYPE_BUFF or self.Type == STYPE_HOT) and self.BuffName ~= "" and
        (self.Target ~= STARGET_FRIENDLY and self.Target ~= STARGET_PARTY) then
    ...
    if (self.Type == STYPE_DAMAGE or self.Type == STYPE_DOT) and self.BuffName ~= "" and target and target.Type ==
        PT_MONSTER then
    ...
    player:updateNature()
    if self.Nature > player.Nature and not player:hasBuff(503817) then -- No need NP if has buff "Unity with Mother Earth"
    ...
    if self.AOECenter and self.MobCount and self.MobCount > 1 then
        ...
        elseif self.AOECenter == SAOE_TARGET and self.AOERange and self.AOERange > 0 then
            if target:findBestClickPoint(self.AOERange, realRange, settings.profile.options.COUNT_AGGRO_ONLY) <
                self.MobCount then
...
function CSkill:use()
    ...
    elseif target and player.Fighting then -- go/stop only when fighting.
        ...
        if settings.profile.options.COMBAT_DISTANCE >= dist and
            (settings.profile.options.COMBAT_STOP_DISTANCE == nil or settings.profile.options.COMBAT_STOP_DISTANCE >=
                dist) then
            keyboardRelease(settings.hotkeys.MOVE_FORWARD.key);
            yrest(200)
        else
        ...
        if target and target:exists() then
            ...
            if self.Type == STYPE_DAMAGE or self.Type == STYPE_DOT then
                if target.Type == PT_MONSTER then
                    local mobcount, x, z = target:findBestClickPoint(self.AOERange, realRange,
                        settings.profile.options.COUNT_AGGRO_ONLY)
...
function CSkill:getRemainingCooldown()
    local secondsSinceLastUse = deltaTime(getTime(), self.LastCastTime) / 1000;
	...
	-- If we have met the cooldown requirement, check what the game client says about the skill
